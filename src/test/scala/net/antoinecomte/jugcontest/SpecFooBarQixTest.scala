 ...
 * 33 contient deux fois 3  et est divisible par 3 donc s'écrit "FooFooFoo"
 """ ! e2 ^
  ...
  import FooBarQix._
  def comp(n: Int) = compute(List(n)).head
  def e1 = List("1", "2", "FooFoo", "4", "BarBar", "Foo", "QixQix", "8", "Foo", "Bar") must_== compute(1 to 10)
  def e2 = List("FooBar", "BarFoo", "FooQix", "Foo", "FooBarBar", "FooFooFoo") must_== compute(List(51, 53, 21, 13, 15, 33))
